...
namespace :deploy do
  PRODUCTION_APP = 'theodinproject'.freeze
  STAGING_APP = '!!!!!YOUR_STAGING_APP_NAME_ON_HEROKU!!!!!'.freeze

  task staging_migrations: %i[set_staging_app push off migrate restart on tag]
  task staging_rollback: %i[set_staging_app off push_previous restart on]

  task production_migrations: %i[set_production_app push off migrate restart on tag]
  task production_rollback: %i[set_production_app off push_previous restart on]
