...
xml.urlset(
  ...
  # Main courses index
  xml.url do
    xml.loc courses_url.to_s
    xml.changefreq('monthly')
    xml.priority(1.00)
  end
  xml.url do
    xml.loc curriculum_url.to_s
    xml.changefreq('monthly')
    xml.priority(0.80)
  end
  ...
  @courses.each do |course|
    xml.url do
      xml.loc course_url(course.title_url).to_s
    ...
    course.lessons.each do |lesson|
      xml.url do
        xml.loc lesson_url(course.title_url, lesson.title_url).to_s
